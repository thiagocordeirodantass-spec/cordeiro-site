...
router.get("/", (req, res) => {
  const rows = db.prepare(`SELECT u.*,eu.empresa_id,eu.permissoes,
    EXISTS(SELECT 1 FROM sessions s WHERE s.user_id=u.id AND datetime(s.expires_at)>datetime('now')
      AND datetime(s.last_seen_at)>datetime('now','-90 seconds')) online FROM users u
